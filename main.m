...
global Blues Yellows Balls Rules FieldInfo RefState RefCommandForTeam RefPartOfFieldLeft RP PAR Modul activeAlgorithm obstacles gameStatus simulator
...
%for grSim------------
if simulator
    linearSpeedFactor = 1/3;
    rotationSpeedFactor = 1;
    for k = 1: 6
        sx = min(100, max(RP.Blue(k).rul.SpeedX, -100));
        sy = min(100, max(RP.Blue(k).rul.SpeedY, -100));
        sr = min(100, max(RP.Blue(k).rul.SpeedR, -100));
        RP.Blue(k).rul.SpeedX = -sy * linearSpeedFactor;
        RP.Blue(k).rul.SpeedY =  sx * linearSpeedFactor;
        RP.Blue(k).rul.SpeedR =  sr * rotationSpeedFactor;
    end
end
%---------------------
