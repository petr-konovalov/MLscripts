function ruls = freeKick(com, ball, kickPoint, BPosHX, BPosHY, obstacles, kickType, kickVoltage)
	...
	switch state
		case 0
			...
			ruls(attackerId) = MoveToConstAcc(MoveToConstAcc(attacker, middlePoint, 0, 95, 10));
		...
		case 1
			...
			if inHalfPlane(attacker.z, ball.z+normir(ball.z-middlePoint)*150, ball.z - middlePoint) && ~inHalfStrip(attacker.z, ball.z, normir(ball.z-middlePoint), 100)
				state = 0;
			end
	end
end
